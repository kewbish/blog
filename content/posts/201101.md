...
```
Then, I can just :+1: react to the message, and the sender knows I've acknowledged and read it, and found it useful.

I wish people would be a bit more cognizant to how email replies work. I've gotten reply-all-ed too many times, and when people inevitably just ask 'it's just a reply all that doesn't even affect you anyway', I'd like to point out that I have email notifications set only so that I receive a notification on new mail, so that I can act on it. I'd rather keep email as a more structured, less Slack-esque form of communication, where I know everything I receive is relevant in some way, and that I consider as important. The same goes for Discord pings, which is a slightly unrelated point.

I wish apps would put more effort into threading: chat apps integrating something like Slack-style threads, for example, though that might not entirely fit the point of the app, I realize. It is also fair to say that if one wants a certain style of communication and reply model that molds how one discusses and communicates, they should just use that app.

## Conclusion
We've grown to adapt our communication to each form of texting and writing that each platform dictates or encourages, and the style that each app works with is highly dependent, I find, on the threading system and how it works. Replies and conversations happen a certain way in real life, face to face, and I feel like each app we use is further evolving how we work on each of the respective apps.
...
It's interesting how I change my speech and communication on each platform, and how odd it is when someone doesn't quite speak or communicate the same way on each platform. I'd never have realized that platforms and what features they implement, could have such a large impact on the way we work, communicate, and discuss.
